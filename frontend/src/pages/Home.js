                ...
                <Row className="mb-3">
                    {categories.map((category) => (
                        <LinkContainer to={`/category/${category.name.toLocaleLowerCase()}`}>
                            <Col md={4}>
                                <div style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url(${category.img})`, gap: "10px" ,height: "500px", backgroundSize: "cover" }} className="category-tile">
                                    {category.name}
                                </div>
                            </Col>
                        </LinkContainer>
